...
#pragma pack(push, 1)	// 0.5 would be much better, but I suppose this will do as it isn't even necessary for our small struct
	...
	struct SMineBackState {
		...
		bool												BlockBased				: 1;
															...
															SMineBackState			() : Blast(false), BlockBased(true) {}		// it's faster to initialize these now than check if they're false by default
	};
...
#pragma pack(pop)
	...
	struct SMineBack {
		::gpk::SImage<::btl::SMineBackCell>					Board					;	// Single-block board mode (deprecated)
